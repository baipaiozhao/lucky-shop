...
// Coupon utilities (shared logic)
export function calcCouponDiscount(totalAmount: number, coupon: { type: string; amount: number; minSpend: number; maxDiscount?: number }): number {
...
export function findBestCoupon(totalAmount: number, coupons: Array<{ id: string; name: string; type: string; amount: number; minSpend: number; maxDiscount?: number }>): { coupon: typeof coupons[number] | null; discount: number } {
  let best: typeof coupons[number] | null = null;
  let bestDiscount = 0;
  for (const c of coupons) { const d = calcCouponDiscount(totalAmount, c); if (d > bestDiscount) { bestDiscount = d; best = c; } }
